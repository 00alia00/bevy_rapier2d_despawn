...
fn spawn_ball(
    ...
    let collider = ColliderBuilder::ball(BALL_RADIUS / RAPIER_SCALE)
        ...
        .user_data((entity.id().to_bits() as u128) << 64 | PhysicsEntityType::Ball as u128);
...
fn spawn_bat(
    ...
    entity.insert(RigidBodyBuilder::new_kinematic().position(Isometry2::new(
        Vector2::new(0.0, -215.0 / RAPIER_SCALE),
        0.0,
    )));
    entity.insert(
        ...
        .restitution(2.0)
        .friction(1.0)
        .user_data(PhysicsEntityType::Bat as u128),
    );
    entity.insert(Bat { speed: 20.0 });
}

fn spawn_wall(
    commands: &mut bevy::prelude::Commands<'_>,
    offset: Vec2,
    size: Vec2,
    ...
    entity.insert(RigidBodyBuilder::new_static().position(Isometry2::new(
        Vector2::new(offset.x / RAPIER_SCALE, offset.y / RAPIER_SCALE),
        0.0,
    )));
    entity.insert(
        ColliderBuilder::cuboid((size.x / RAPIER_SCALE) / 2.0, (size.y / RAPIER_SCALE) / 2.0)
            .restitution(1.0) // Bouncy
            .friction(0.0) // No friction
            // Set the user data: first 64 bits is the entity type and the second 64 bits is the entity ID
...
fn setup_walls(
    ...
    spawn_wall(
        &mut commands,
        Vec2::new(-bounds.x / 2.0, 0.0),
        Vec2::new(wall_thickness, bounds.y + wall_thickness),
        &wall_material,
    );
    spawn_wall(
        &mut commands,
    ...
    spawn_wall(
        &mut commands,
        Vec2::new(0.0, -bounds.y / 2.0),
        Vec2::new(bounds.x + wall_thickness, wall_thickness),
        &wall_material,
    );
    spawn_wall(
        &mut commands,
...
fn spawn_bricks(
    ...
    for row in 0..brick_rows {
        ...
        for column in 0..brick_columns {
            ...
            entity.insert(RigidBodyBuilder::new_static().position(Isometry2::new(
                Vector2::new(
                    brick_position.x / RAPIER_SCALE,
                    brick_position.y / RAPIER_SCALE,
                ),
                0.0,
            )));
    ...
    while let Ok(contact_event) = events.contact_events.pop() {
        match contact_event {
            ContactEvent::Started(c0, c1) => {
                ...
                if t0bits != PhysicsEntityType::Wall as u64
                    && t1bits != PhysicsEntityType::Wall as u64
                {
